#Helper functions for scouting app
...
basic_table <- function(df){
  pa_types <- df %>% 
    count(events) %>% 
    filter(events %!in% c("caught_stealing_2b", "caught_stealing_3b", "caught_stealing_home", "catcher_interference", "sac_bunt", NA))
...
hotzones <- function(df){
  ...
  slg_zone <- function(pzone, df_full){
    ...
    pa_types <- df_zone %>% 
      count(events) %>% 
      filter(events %!in% c("caught_stealing_2b", "caught_stealing_3b", "caught_stealing_home", "catcher_interference", NA))
